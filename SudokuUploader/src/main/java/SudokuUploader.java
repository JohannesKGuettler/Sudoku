...
public class SudokuUploader implements AutoCloseable {
     ...
     * @param dailySudoku: array of array of integers that represents sudoku grid, only non empty if method is used in testing
     */
    public void uploadSudokus(int amountDays, String sourceDate, int[][] testSudoku) {
    	Generator SudokuGenerator;
    	String date;
    	int[][] dailySudoku = new int[9][9];
    	...
    	for (int i=0; i<amountDays; i++) {
    		...
    		try (var session = driver.session()) {
    			...
    			if (testSudoku.length == 0) {
	            	...
	            	dailySudoku = SudokuGenerator.getSudoku();
	            	this.type = SudokuGenerator.getType();
    			}
    			// set dailySudoku to testSudoku if method is used for testing
    			else {
    				dailySudoku = testSudoku;
    			}
            	...
            	String qq = "CREATE (a:Sudoku {uuid:" + id + ", date : " + date + ",type :" + this.type + ", row1:" + Arrays.toString(dailySudoku[0]) + ", row2:" + Arrays.toString(dailySudoku[1]) + ", row3:" + Arrays.toString(dailySudoku[2]) + ", row4:" + Arrays.toString(dailySudoku[3]) + ", row5:" + Arrays.toString(dailySudoku[4]) + ", row6:" + Arrays.toString(dailySudoku[5]) + ", row7:" + Arrays.toString(dailySudoku[6]) + ", row8:" + Arrays.toString(dailySudoku[7]) + ", row9:" + Arrays.toString(dailySudoku[8]) + "})";
    ...
    public int[][] getSudoku(String date) {
        ...
        try (var session = driver.session(SessionConfig.forDatabase("neo4j"))) {
            var record = session.executeRead(tx -> tx.run(query).next()); // method would need to check a possible exception here if no sudokus exist for the given date. But we were unable to figure out how to code it. Multiple values are not a problem through the usage of next()
    ...
    public static void main(String... args) {
        	...
        	int[][] sudoku = new int[][] {};
        	greeter.uploadSudokus(4, "2019-06-19", sudoku);
        	greeter.close();
    }
}
