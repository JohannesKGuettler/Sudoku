...
public class Startpage extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
        ...
        scoresButton.setOnAction(event -> {
            ...
            if (!sudokuCompleted) {
                ...
                Server greeter = new Server("neo4j+s://72b6cafc.databases.neo4j.io", "neo4j", "lj6AD4ac4CFidRzRqtkeIzfKabcaYppX6lfIPzhZzkQ");
	...
	private void loadData(LocalDate date) {
		...
		Server greeter = new Server("neo4j+s://1a2d3c68.databases.neo4j.io", "neo4j", "lj6AD4ac4CFidRzRqtkeIzfKabcaYppX6lfIPzhZzkQ");
    ...
    private void setNumber(int number) {
        ...
        if (selectedTextField != null && selectedTextField.isEditable()) {
            ...
            if (isValidSudoku(row, column, number, grid, type)) {
                if (category.equals("numbers")) {
                ...
                if (isSudokuCompleted(grid)) {   
                    ...
                    toScoresPageButton.setOnAction(event -> { 
                    	...
                    	Server greeter = new Server("neo4j+s://1a2d3c68.databases.neo4j.io", "neo4j", "fwYW8GEj5aCLmT5PMisyOO-pMmsQckUyO3qWrluxDa8");
     ...
     * @param sudoku array of arrays of integers that represents sudoku grid
     * @param typeToday type of todays sudoku
     * @return true If the grid is valid, false otherwise
     */
    public boolean isValidSudoku(int row, int column, int number, int[][] sudoku, int typeToday) {
    	int [] rowColumn = new int[] {row, column};
    	// check if number is already in row
    	for(int i=0; i<9; i++) {
    		if (sudoku[row][i] == number & i != rowColumn[1]) {
    			return false;
   			}
    	}
    	...
    	for (int i=0; i<9; i++) {
   			if (sudoku[i][column] == number & i != rowColumn[0]) {
    			return false;
    		}
    	}
    	...
    	for (int i=0; i<3; i++) {
    		for (int k=0; k<3; k++) {
    			...
    			if (sudoku[box[0]*3 + i][box[1]*3 + k] == number & !Arrays.equals(rowColumn, checkedPosition)) {
    				return false;
    			}
    		}
    	}
    	...
    	if (typeToday == 1) {
    		if (row == 4 & column == 4) {
    			for (int i=0; i<9; i++) {
    				if ((sudoku[i][i] == number | sudoku[8-i][i] == number) & i != rowColumn[0]) {
    					return false;
    				}
    			}
    		}
    			...
    			for (int i=0; i<9; i++) {
    				if (sudoku[i][i] == number & i != rowColumn[0]) {
    					return false;
    				}
    			}
    		}
    			...
    			for (int i=0; i<9; i++) {
    				...
    				checkedPosition[1] = i;
    				if (sudoku[8-i][i] == number & !Arrays.equals(rowColumn, checkedPosition)) {
    					return false;
    				}
    			}
    		}
     ...
     *
     *@param sudoku arrays of arrays of integers that represents sudoku grid
     * @return true If the sudoku is completed, false if there is any empty cell
     */
    public boolean isSudokuCompleted(int[][] sudoku) {
